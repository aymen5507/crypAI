...
def preprocess(dataset, cfg, logger=None):
    ...
    dataset, profit_calc = create_dataset(arr, list(dates), look_back=cfg.dataset_loader.window_size, features=features)
    return dataset, profit_calc
...
def create_dataset(dataset, dates, look_back, features):
    ...
    profit_calc = profit_calc.rename({f'High_day{counter_date - 1}': 'High',
